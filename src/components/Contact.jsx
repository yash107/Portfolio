...
const socials = [
  ...
  {
    name: 'LinkedIn',
    href: 'https://www.linkedin.com/in/yash-sachdeva-9214ba223/',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className={`md:col-span-2 transition-all duration-700 delay-100 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            ...
            <div className="space-y-3 mb-8">
              ...
              <span className="flex items-center gap-3 font-mono text-xs text-ink-500 dark:text-ink-400">
                <span className="text-accent">◉</span> Bengaluru, India
              </span>
            </div>
            <div>
              ...
              <form onSubmit={submit} className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input name="name" value={form.name} onChange={handle} required placeholder="Elon Musk" className={inputCls} />
                  </div>
                  <div>
                    <label className="block font-mono text-[10px] tracking-widest uppercase text-ink-400 dark:text-ink-500 mb-1.5">Email</label>
                    <input type="email" name="email" value={form.email} onChange={handle} required placeholder="elon.musk@spacex.com" className={inputCls} />
                  </div>
                </div>
                <div>
                  <label className="block font-mono text-[10px] tracking-widest uppercase text-ink-400 dark:text-ink-500 mb-1.5">Message</label>
                  <textarea name="message" value={form.message} onChange={handle} required rows={5} placeholder="Come and join us yash!" className={`${inputCls} resize-none`} />
                </div>
